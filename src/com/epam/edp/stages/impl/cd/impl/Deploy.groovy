...
class Deploy {
    ...
    def getRefspec() {
        if (codebase.versioningType == "edp") {
            return refspec = "refs/tags/build/${codebase.branch}-${codebase.version}"
        }
        return refspec = "refs/tags/build/${codebase.version}"
    }
    script.println("[JENKINS][DEBUG] getRefspec: ${refspec}")
