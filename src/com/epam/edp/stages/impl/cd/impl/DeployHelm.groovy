...
class DeployHelm {
    ...
    def cloneProject(context, codebase) {
        ...
        try {
            script.checkout([$class                           : 'GitSCM', branches: [[name: "refs/tags/build/${context.codebase.version}--${script.BUILD_NUMBER}"]],
                             doGenerateSubmoduleConfigurations: false, extensions: [],
                             submoduleCfg                     : [],
                             userRemoteConfigs                : [[credentialsId: "${credentialsId}",
                                                                  refspec      : "refs/tags/build/${context.codebase.version}--${script.BUILD_NUMBER}",
                                                                  url          : "${gitCodebaseUrl}"]]])
        }
        catch (Exception ex) {
