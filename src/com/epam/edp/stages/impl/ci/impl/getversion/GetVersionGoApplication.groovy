...
class GetVersionGoApplication {
    ...
    def setUpVersions(workDir, codebase, git, job) {
        ...
        /*def dockerfilePath = new FilePath(new File("${workDir}/Dockerfile"))
        dockerfilePath.write(dockerfileContents, null)*/

        script.println("TEST111 ->>>>> ${workDir}/VERSION")
        String str = "${workDir}/VERSION"
        def version = readFile(str)
        script.println("[JENKINS][DEBUG] versionversionversionversion ${version}")
