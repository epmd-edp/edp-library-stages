...
class SonarNpmApplicationLibrary {
    ...
    def sendSonarScan(sonarProjectName, codereviewAnalysisRunDir, credentialsId) {
        script.dir("${codereviewAnalysisRunDir}") {
    ...
    void run(context) {
        ...
        if (context.job.type == "codereview") {
            sendSonarScan("${context.codebase.name}:change-${context.git.changeNumber}-${context.git.patchsetNumber}", codereviewAnalysisRunDir)
            getSonarReportJson(context, codereviewAnalysisRunDir)
        } else {
           sendSonarScan(context.codebase.name, codereviewAnalysisRunDir)
        }
        script.timeout(time: 10, unit: 'MINUTES') {
