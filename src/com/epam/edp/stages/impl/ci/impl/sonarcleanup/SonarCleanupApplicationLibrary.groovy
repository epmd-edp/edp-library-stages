...
class SonarCleanupApplicationLibrary {
    def cleanSonarProjectRange(patchsetNumber, sonarRoute, sonarProjectKey) {
    ...
    void run(context) {
        script.withSonarQubeEnv('Sonar') {
            ...
            if (context.git.patchsetNumber != "0" && context.git.changeNumber != "0") {
                cleanSonarProjectRange(context.git.patchsetNumber, context.sonar.route, sonarProjectKey)
            }
        }
    }
}