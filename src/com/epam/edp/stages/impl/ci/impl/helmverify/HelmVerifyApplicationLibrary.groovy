...
class HelmVerifyApplicationLibrary {
    ...
    void run(context) {
        def helmLatestVersion = script.sh(
            script: "curl -Ls https://github.com/helm/helm/releases | grep 'href=\"/helm/helm/releases/tag/v3.[0-9]*.[0-9]*\"' | grep -v no-underline | head -n 1 | cut -d '\"' -f 2 | awk '{n=split(\$NF,a,\"/\");print a[n]}' | awk 'a !~ \$0{print}; {a=\$0}'",
            returnStdout: true
        ).trim()
        def helmVersion = context.job.getParameterValue("HELM_VERSION", helmLatestVersion)
        script.print(helmVersion)
        script.dir("${context.workDir}") {
            script.sh """
            curl -o helm.tar.gz https://get.helm.sh/helm-${helmVersion}-linux-amd64.tar.gz
