...
class HelmVerifyApplicationLibrary {
    ...
    void run(context) {
        helmLatestVersion = script.sh """curl -Ls https://github.com/helm/helm/releases | grep 'href="/helm/helm/releases/tag/v3.[0-9]*.[0-9]*"' | grep -v no-underline | head -n 1 | cut -d '"' -f 2 | awk '{n=split(\$NF,a,"/");print a[n]}' | awk 'a !~ \$0{print}; {a=\$0}'"""
        helmVersion = context.job.getParameterValue("HELM_VERSION", "${helmLatestVersion}")
