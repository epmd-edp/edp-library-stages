...
class JiraFixVersion {
    ...
    def parseJiraFixVersionTemplate(template, context, commits, pattern) {
        ...
        template.metadata.name = "${context.codebase.config.name}-${context.codebase.isTag}"
        template.metadata.ownerReferences[0].name = context.codebase.config.name
        template.metadata.ownerReferences[0].uid = context.platform.getJsonPathValue("codebases.v2", context.codebase.config.name, ".metadata.uid")
    ...
    def tryToCreateJiraFixVersionCR(workDir, platform, parsedTemplate) {
        if (new JsonSlurperClassic().parseText(parsedTemplate).spec.tickets == "replace") {
            script.println("[JENKINS][DEBUG] NO CHANGES")
        } else {
            script.println("[JENKINS][DEBUG] CHANGES")
