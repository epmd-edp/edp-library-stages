...
class JiraFixVersion {
    ...
    def getTickets(workDir, pattern) {
        script.dir("${workDir}") {
            ...
            for (commit in changes.getCommits()) {
                def names = commit.getCommitInfo().getCommitMessage().findAll(pattern)
                tickets.addAll(names)
            }
            return tickets
        }
    }
    ...
    void run(context) {
        def tickets = getTickets(context.workDir, context.codebase.config.ticketNamePattern)
