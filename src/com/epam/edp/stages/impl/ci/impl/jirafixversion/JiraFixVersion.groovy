...
class JiraFixVersion {
    ...
    def parseJiraFixVersionTemplate(template, name, commits, pattern) {
    ...
    void run(context) {
        ...
        if (commits == null) {
            ...
            def parsedTemplate = parseJiraFixVersionTemplate(template,
                    "${context.codebase.config.name}-${context.codebase.isTag}", commits, ticketNamePattern)


            if (template == new JsonSlurperClassic().parseText(parsedTemplate)) {
