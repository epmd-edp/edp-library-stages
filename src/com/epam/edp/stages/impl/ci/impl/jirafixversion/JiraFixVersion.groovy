...
class JiraFixVersion {
    ...
    def getJiraFixTemplate(platform) {
        ...
        return new JsonSlurperClassic().parseText(platform.getJsonPathValue("cm", "jfv-template", ".data.jfv\\.json"))
    }
    ...
    def addTicketNumber(template, ticket) {
        if (template.spec.tickets == "replace") {
            template.spec.tickets = []
        }
        template.spec.tickets.add(ticket)
    }
    ...
    void run(context) {
        ...
        def parsedTemplate = parseJiraFixVersionTemplate(template,
                "${context.codebase.config.name}_${context.codebase.isTag}", changes, ticketNamePattern)
