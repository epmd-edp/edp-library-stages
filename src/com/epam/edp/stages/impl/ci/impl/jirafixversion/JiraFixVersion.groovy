...
class JiraFixVersion {
    ...
    def parseJiraFixVersionTemplate(template, context, commits, pattern) {
        ...
        template.ownerReferences[0]['name'] = context.codebase.config.name
        template.ownerReferences[0]['uid'] = context.platform.getJsonPathValue("codebases.v2", context.codebase.config.name, ".metadata.uid")
