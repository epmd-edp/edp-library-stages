...
class GetVersionNpmApplicationLibrary {
    ...
    def updateCodebaseBuildNumber(buildNumber, context) {
        buildNumber++
        script.sh"""
            kubectl patch codebasebranches.v2.edp.epam.com ${context.codebase.config.name}-${context.git.branch} --type=merge -p '{\"spec\": {\"codebase_branch\": [\"build_number\": ${buildNumber}]}}'
        """
    }
