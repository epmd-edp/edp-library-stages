...
class GetVersionNpmApplicationLibrary {
    ...
    void run(context) {
        script.dir("${context.workDir}") {
            ...
            if (context.codebase.versioning_type == "edp") {
                context.codebase.version = script.sh(script: """
                    sed -i "/version/c/\\  \\"version\\": \\"${context.codebase.versioning.startFrom}-${context.codebase.build_number}\\"," package.json
                    echo "${context.codebase.versioning.startFrom}-${context.codebase.build_number}"
                """)
