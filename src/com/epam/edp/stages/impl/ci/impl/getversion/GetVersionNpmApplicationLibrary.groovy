    ...
    void run(context) {
        script.dir("${context.workDir}") {
            ...
            if (context.codebase.config.versioningType == "edp") {
                ...
                script: """
                    sed -i "/version/c\\  \\"version\\": \\"${context.codebase.config.versioning.startFrom}-${build_number}\\"," package.json
                    echo "${context.codebase.config.versioning.startFrom}-${build_number}"
                """, returnStdout: true
                ).trim().toLowerCase()
