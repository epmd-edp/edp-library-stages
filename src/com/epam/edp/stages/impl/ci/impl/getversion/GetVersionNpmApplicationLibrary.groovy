...
class GetVersionNpmApplicationLibrary {
    ...
    def updateCodebaseBranchCR(buildNumber, newVersion, context) {
        def nextBuildNumber = buildNumber++
        script.sh"""
            kubectl patch codebasebranches.v2.edp.epam.com ${context.codebase.config.name}-${context.git.branch} --type=merge -p '{\"spec\": {\"build\": "${nextBuildNumber}", \"version\": "${newVersion}"}}'
        """
    }
