...
class GetVersionNpmApplicationLibrary {
    ...
    def updateCodebaseBranchCR(buildNumber, newVersion, context) {
        def newBuildNumber = ++buildNumber
        script.sh"""
            kubectl patch codebasebranches.v2.edp.epam.com ${context.codebase.config.name}-${context.git.branch} --type=merge -p '{\"spec\": {\"build\": "${newBuildNumber}", \"version\": "${newVersion}"}}'
        """
    }
    ...
    void run(context) {
        script.dir("${context.workDir}") {
            if (context.codebase.config.versioningType == "edp") {
                script.println (context.codebase.config.codebase_branch.build_number.get(0))
                context.codebase.version = setVersionToArtifact(context.codebase.config.codebase_branch.build_number.get(0), context)
