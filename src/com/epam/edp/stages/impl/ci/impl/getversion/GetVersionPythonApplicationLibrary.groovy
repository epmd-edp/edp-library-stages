...
class GetVersionPythonApplicationLibrary {
    ...
    def setVersionToArtifact(context) {
        script.dir("${context.workDir}") {
            ...
            script.sh "kubectl patch codebasebranches.v2.edp.epam.com ${context.codebase.config.name}-${context.git.branch.replaceAll(/\//, "-")} --type=merge -p '{\"status\": {\"build\": "${context.codebase.buildVersion}"}}'"
        }
    }
