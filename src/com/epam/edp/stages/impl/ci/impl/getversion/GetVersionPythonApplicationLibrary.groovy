...
class GetVersionPythonApplicationLibrary {
    ...
    def setVersionToArtifact(workDir) {
        script.dir("${workDir}") {
            script.sh (script """sed -i 's/\\(__version__\\s*=\\s*\\).*/\\1\"${context.codebase.buildVersion}\"/' version/__init__.py""")
        }
    }
    ...
    void run(context) {
        ...
        setVersionToArtifact(context.workDir)
