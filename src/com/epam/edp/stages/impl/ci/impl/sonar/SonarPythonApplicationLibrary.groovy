...
class SonarPythonApplicationLibrary {
    ...
    def getSonarReportInJson(workDir, url) {
        script.httpRequest acceptType: 'APPLICATION_JSON',
                url: url,
                httpMode: 'GET',
                outputFile: "target/sonar/sonar-report.json/sonar-report.json"
    }

    def sendStatusToGerrit(workDir, sonarURL) {
        script.dir("${workDir}") {
            script.sonarToGerrit inspectionConfig: [baseConfig: [projectPath: "", sonarReportPath: "target/sonar/sonar-report.json/sonar-report.json"], serverURL: "${sonarURL}"],
