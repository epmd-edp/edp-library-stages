...
class BuildDockerfileImageApplication {
    ...
    void createOrUpdateBuildConfig(codebase, buildConfigName, imageUrl) {
        ...
        /*def patcher = [spec:[output:[to:["name":imageUrl]]]]
        script.sh("oc patch --type=merge ${buildConfigApi} ${buildConfigName} -p ${JsonOutput.toJson(patcher)}")*/
        def asd = "oc patch --type=merge ${buildConfigApi} ${buildConfigName} -p \"{\\\"spec\\\":{\\\"output\\\":{\\\"to\\\":{\\\"name\\\":\\\"${imageUrl}\\\"}}}}\" "
        script.println("[JENKINS][DEBUG] asd  ${asd}")
        script.sh(asd)
    }
