...
class BuildDockerfileImageApplication {
    ...
    void run(context) {
        ...
        def buildconfigName = "${context.codebase.name}-dockerfile-${context.git.displayBranch.replaceAll("[^\\p{L}\\p{Nd}]+", "-")}"
        def outputImagestreamName = "${context.codebase.name}-${context.git.displayBranch.replaceAll("[^\\p{L}\\p{Nd}]+", "-")}"
