...
class SonarMaven {
    ...
    def getSonarReportJson(context, codereviewAnalysisRunDir) {
        ...
        sendReport(context, codereviewAnalysisRunDir, sonarReportMap.serverUrl)
    }

    def sendReport(context, codereviewAnalysisRunDir, sonarReportMap.serverUrl) {
            script.println("[JENKINS][DEBUG] SONAR URL - ${sonarReportMap.serverUrl}")
            script.sonarToGerrit inspectionConfig: [baseConfig: [projectPath: "", sonarReportPath: "/target/sonar/sonar-report.json"], serverURL: "${sonarReportMap.serverUrl}"],
