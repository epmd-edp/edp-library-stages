...
class SonarMaven {
    ...
    void run(context) {
        ...
        if (context.job.type == "codereview") {
           def projectKey = ${context.codebase.name}:change-${context.git.changeNumber}-${context.git.patchsetNumber}
           def projectName = ${context.codebase.name}:change-${context.git.changeNumber}-${context.git.patchsetNumber}
           ...
           def projectKey = ${context.codebase.name}
           def projectName = ${context.codebase.name}
        }
        script.dir("${codereviewAnalysisRunDir}") {
            script.withSonarQubeEnv('Sonar') {
                script.withCredentials([script.usernamePassword(credentialsId: "${context.nexus.credentialsId}",
