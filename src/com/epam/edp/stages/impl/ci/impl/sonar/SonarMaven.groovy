...
class SonarMaven {
    ...
    def getSonarReportJson(context, codereviewAnalysisRunDir) {
        ...
        def sonarUrl = "${sonarReportMap.serverUrl}"
        ...
        sendReport(context, codereviewAnalysisRunDir, sonarUrl)
    }

    def sendReport(context, codereviewAnalysisRunDir, sonarUrl) {
            script.println("[JENKINS][DEBUG] SONAR URL - ${sonarUrl}")
            script.sonarToGerrit inspectionConfig: [baseConfig: [projectPath: "", sonarReportPath: "sonar-report.json"], serverURL: "${sonarUrl}"],
