...
class SonarMaven {
    ...
    def getSonarReportJson(context, codereviewAnalysisRunDir) {
        ...
        def sonarJsonReportLink = "${context.sonar.route}/api/issues/search?componentKeys=${context.codebase.name}-${context.git.changeName}&branch=${context.git.branch}&resolved=false&facets=severities"

        script.println("[JENKINS][DEBUG] Sonar ProjectKey - ${context.codebase.name}-${context.git.changeName}")
    ...
    def sendReport(context, codereviewAnalysisRunDir) {
        script.dir("${codereviewAnalysisRunDir}") {
            ...
            script.sonarToGerrit inspectionConfig: [baseConfig: [projectPath: "", sonarReportPath: "target/sonar/sonar-report.json"], serverURL: "${context.sonar.route}"],
                    ...
                    reviewConfig: [issueFilterConfig: [newIssuesOnly: false, changedLinesOnly: false, severity: 'INFO']],
                    scoreConfig: [category: 'Code-Review', noIssuesScore: 0, issuesScore: -1, issueFilterConfig: [severity: 'INFO']]
        }
    }
            ...
            script.withSonarQubeEnv('Sonar') {
                script.withCredentials([script.usernamePassword(credentialsId: "${context.nexus.credentialsId}",
                    ...
                    script.sh "${context.buildTool.command} ${context.buildTool.properties} -Dartifactory.username=${script.USERNAME} -Dartifactory.password=${script.PASSWORD} " +
                            "sonar:sonar " +
                            "-Dsonar.projectKey=${context.codebase.name}-${context.git.changeName} " +
                            "-Dsonar.projectName=${context.codebase.name}-${context.git.changeName} " +
                            "${context.job.type == "codereview" ? validate : context.git.branch}"
                }
            }
            script.timeout(time: 10, unit: 'MINUTES') {
