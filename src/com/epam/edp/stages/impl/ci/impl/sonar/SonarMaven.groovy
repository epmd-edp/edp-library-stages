...
class SonarMaven {
    ...
    def runSonarScannerDependsOnPlatform(context, codebase, platform, codereviewAnalysisRunDir) {
        if (platform == "kubernetes" || codebase.strategy == "import") {
    ...
    void run(context) {
        ...
        if (context.job.type == "codereview") {
            ...
            runSonarScannerDependsOnPlatform(context, codebase, System.getenv("PLATFORM_TYPE"), codereviewAnalysisRunDir)
        } else {
            sendSonarScan(context.codebase.name, codereviewAnalysisRunDir, context.buildTool, context.nexus.credentialsId)
        }
        script.timeout(time: 10, unit: 'MINUTES') {
