...
class SonarMaven {
    ...
    def run(vars) {
        def runDir = vars.containsKey('codereviewAnalysisRunDir') ? vars['codereviewAnalysisRunDir'] : vars['workDir']
        dir("${runDir}") {
            getSonarReportJson("${vars.itemMap.name}:${vars.gerritChange}", vars.sonarPrecommitDir, vars.serviceBranch, vars.sonarUrl)

            sonarToGerrit inspectionConfig: [baseConfig: [projectPath: "", sonarReportPath: "${vars.sonarPrecommitDir}/sonar-report.json"], serverURL: "${vars.sonarRoute}"],
