...
class SonarDotnetApplicationLibrary {
    ...
    def checkPlatform(context, platform, codereviewAnalysisRunDir, scannerHome) {
    ...
    void run(context) {
        ...
        if (context.job.type == "codereview") {
            checkPlatform(context, System.getenv("PLATFORM_TYPE"), codereviewAnalysisRunDir, scannerHome)
        } else {
            sendSonarScan(context.codebase.name, codereviewAnalysisRunDir, context.buildTool, scannerHome)
        }
        script.timeout(time: 10, unit: 'MINUTES') {
