...
class SonarDotnetApplicationLibrary {
    def getSonarReportJson(context) {
        String sonarAnalysisStatus
        def sonarReportMap = script.readProperties file: "${context.workDir}/.sonarqube/out/.sonar/report-task.txt"
        ...
        script.println("[JENKINS][DEBUG] RUN DIR - ${context.workDir}")
        ...
        if (context.job.type == "codereview") {
            script.dir("${context.workDir}") {
                script.withSonarQubeEnv('Sonar') {
                    script.sh """
                    dotnet ${scannerHome}/SonarScanner.MSBuild.dll begin /k:${context.codebase.name}:change-${context.git.changeNumber}-${context.git.patchsetNumber} \
