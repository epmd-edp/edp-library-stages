...
class SonarDotnetApplicationLibrary {
    ...
    def sendReport(sonarURL, codereviewAnalysisRunDir) {
        script.dir("${codereviewAnalysisRunDir}") {
            script.sonarToGerrit inspectionConfig: [baseConfig: [projectPath: "${codereviewAnalysisRunDir}", sonarReportPath: "${context.workDir}/.sonarqube/out/.sonar/sonar-report.json"], serverURL: "${sonarURL}"],
                    notificationConfig: [commentedIssuesNotificationRecipient: 'NONE', negativeScoreNotificationRecipient: 'NONE'],
                    reviewConfig: [issueFilterConfig: [newIssuesOnly: true, changedLinesOnly: true, severity: 'MAJOR']],
                    scoreConfig: [category: 'Sonar-Verified', noIssuesScore: +1, issuesScore: -1, issueFilterConfig: [severity: 'MAJOR']]
        }
    }
