...
class SonarDotnetApplicationLibrary {
    ...
    def sendReport(sonarURL, codereviewAnalysisRunDir) {
        script.dir("${codereviewAnalysisRunDir}") {
            script.sonarToGerrit inspectionConfig: [autoMatch: true, baseConfig: [autoMatch: true, projectPath: "${codereviewAnalysisRunDir}", sonarReportPath: "target/sonar/sonar-report.json"], serverURL: "${sonarURL}"],
