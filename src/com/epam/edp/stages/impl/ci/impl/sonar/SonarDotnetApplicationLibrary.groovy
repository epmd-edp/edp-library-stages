...
class SonarDotnetApplicationLibrary {
    ...
    def sendSonarScan(sonarProjectName, codereviewAnalysisRunDir, buildTool, scannerHome) {
        script.dir("${codereviewAnalysisRunDir}") {
                 script.withSonarQubeEnv('Sonar') {
                     script.sh """
                     ${scannerHome}/SonarScanner.MSBuild.dll begin /k:${sonarProjectName} \
                     /k:${sonarProjectName} \
                     /n:${sonarProjectName} \
    ...
    void run(context) {
        ...
        def scannerHome = context.codebase.config.framework == "netcore" ? "dotnet script.tool 'SonarScannerMSBuild'" : "dotnet /home/jenkins/.dotnet/tools/dotnet-sonarscanner"
